...
const LOCATION_TOO_LONG_MESSAGE = 'Location too long'
...
export const createVisitSchema = z
  .object({
    ...
    duration: positiveIntSchema
      .min(15, 'Visit must be at least 15 minutes')
      .max(480, 'Visit cannot exceed 8 hours')
      .optional(),
...
export const visitCheckinSchema = z.object({
  ...
  arrivalMethod: z
    .enum(['walking', 'driving', 'public_transport', 'other'])
    .optional(),
})

// Visit check-out schema
export const visitCheckoutSchema = z
  .object({
    activities: activitiesArraySchema,
    notes: z.string().max(1000, 'Visit notes too long').optional(),
    vitals: vitalsSchema.optional(),
    medications: z
      .array(z.string().max(100, 'Medication name too long'))
    ...
    incidentDescription: z
      .string()
      .max(500, 'Incident description too long')
      .optional(),
  })
  .refine(
    (data) => {
    ...
    {
      message:
        'Follow-up reason required when follow-up is needed, incident description required when incident occurred',
    }
  )
...
export const rescheduleVisitSchema = z
  .object({
    ...
    reason: z
      .string()
      .min(1, 'Reschedule reason required')
      .max(200, REASON_TOO_LONG_MESSAGE),
    notifyParties: z.boolean().default(true),
  })
  .refine(
    (data) => {
...
export const bulkVisitActionSchema = z.object({
  visitIds: z
    .array(cuidSchema)
    .min(1, 'At least one visit ID required')
    .max(50, 'Too many visits selected'),
