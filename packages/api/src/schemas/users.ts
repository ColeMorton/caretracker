...
export const userProfileSchema = z.object({
  ...
  primaryCarePhysician: z
    .string()
    .max(100, 'Physician name too long')
    .optional(),
  ...
  emergencyContactAddress: z
    .string()
    .max(200, 'Emergency contact address too long')
    .optional(),

  // Healthcare data arrays
  allergies: z
    .array(z.string().max(100, 'Allergy description too long'))
  ...
  medications: z
    ...
    .default([]),
  medicalConditions: z
    .array(z.string().max(100, 'Condition description too long'))
    .max(30, 'Too many conditions listed')
    .default([]),
  specialNeeds: z
    .string()
    .max(500, 'Special needs description too long')
    .optional(),
...
export const workerAssignmentSchema = z.object({
  workerIds: z
    .array(cuidSchema)
    .min(1, 'At least one worker required')
    .max(50, 'Too many workers'),
  clientIds: z
    .array(cuidSchema)
    .min(1, 'At least one client required')
    .max(100, 'Too many clients'),
})

// Route import aliases (defined after schemas to avoid forward reference errors)
